...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  void createAdjacencyLists( int[][] edges, int numberOfVertices) {
    for (int i = 0; i < edges.length; i++) addEdge(edges[i][0], edges[i][1]);
    for (int [] e: edges) System.out.println(java.util.Arrays.toString(e));
    }

  /** Create adjacency lists for each vertex */
  void createAdjacencyLists( List<Edge> edges, int numberOfVertices) { 
    edges.forEach(i -> addEdge(i.u, i.v));
    edges.forEach(i -> System.out.print("{" + i.u + ", " + i.v + "},"));
    }
